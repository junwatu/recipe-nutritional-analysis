...
React is used to build the user interface. There is only one file, `App.jsx`, which main purpose is user input for food recipes and displaying the nutrition analysis data.
...
const App = () => {
    ...
    const handleAnalyze = async () => {
        ...
        setMarkdown('')
        try {
            const response = await axios.post(`${import.meta.env.VITE_APP_URL}/analyze`, {
                ingredients: recipe
            })
